...
static void debug_exec(char *name, int argc, const char **argv) {
    LOG_TRACE("exec name=%s, argc=%d", name, argc);
...
int exec(char *name, int argc, const char **argv) {
    ...
    // LOG_TRACE("trapframe->sp=%p", p->trapframe->sp);
    // LOG_TRACE("trapframe->a0=%p", p->trapframe->a0);
    // LOG_TRACE("trapframe->a1=%p", p->trapframe->a1);
